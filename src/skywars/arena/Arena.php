...
class Arena implements Listener {
    ...
    public function joinToArena(Player $player) {
		...
		$player->sendMessage("§c> Use §a[/spawn | /hub]§c to leave.");
    }
	 ...
	 *
	 */
	private function giveRewards($player): void {
		$pm = $this->plugin->getServer()->getPluginManager();
		$to = (string) $player->getLowerCaseName();
		$pm->getPlugin("EconomyAPI")->addMoney($to, 500);
		$pm->getPlugin("LevelAPI")->addVal($to, "gems", mt_rand(13, 15));
		
		$player->sendMessage("§a> You received 500 cash");
	}
    ...
    */
    public function onRespawn(PlayerRespawnEvent $event) {
        ...
        if(isset($this->toRespawn[$player->getName()])) {
			...
			#$event->setRespawnPosition($player->getPosition());
    ...
    public function onLevelChange(EntityLevelChangeEvent $event) {
        ...
        if(!$player instanceof Player) return;
        if($this->inGame($player)) {
            $this->disconnectPlayer($player, "You successfully left the arena!");
        }
    }
    ...
    public static function getChestItems(): array {
        ...
        $chestItems[1] = [
            298, 299, 300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310, 311, 312, 313, 314, 315, 316, 317, 261
        ];
        $chestItems[2] = [
            319, 320, 297, 391, 392, 393, 396, 400, 411, 412, 423, 424, 263
        ];
        $chestItems[3] = [
            1, 2, 3, 4, 5, 12, 13, 14, 15, 16, 17, 18, 82, 35, 45, 262
        ];
        $chestItems[4] = [
            262, 263, 264, 265, 266, 280, 297, 322, 263
        ];
        return $chestItems;
    }
