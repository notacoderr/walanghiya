        ...
        switch ($this->plugin->phase) {
            case Arena::PHASE_LOBBY:
                if(count($this->plugin->players) >= 2) {
                    ...
                    $this->plugin->broadcastMessage("§c> Waiting for more Players ", Arena::MSG_TIP);
                    $this->startTime = 40;
                }
                break;
            case Arena::PHASE_GAME:
                $this->plugin->broadcastMessage("§a> Players: " . count($this->plugin->players) . ", time remaining: " . Time::calculateTime($this->gameTime) . "", Arena::MSG_TIP);
            ...
            case Arena::PHASE_RESTART:
                ...
                switch ($this->restartTime) {
					...
					/*case 9:
						...
						foreach ($this->plugin->spectator as $spectator) {
                            $spectator->teleport($this->plugin->plugin->getServer()->getDefaultLevel()->getSpawnLocation());

                            $spectator->getInventory()->clearAll();
                            $spectator->getArmorInventory()->clearAll();

                            $spectator->setFood(20);
                            $spectator->setHealth(20);

                            $spectator->setGamemode($this->plugin->plugin->getServer()->getDefaultGamemode());
                        }
